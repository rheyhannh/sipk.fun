...
export async function rateLimit(options) {
	...
	const max =
		typeof uniqueTokenPerInterval === 'number' && uniqueTokenPerInterval > 0
	...
	const check = async (limit, token) => {
				...
				return Promise.reject(
					rateLimitError.maximum_token(
						...
						undefined,
						{
							...
							stack: null,
							functionDetails:
								'Ratelimit.check at utils/server_side.js line 143',
							functionArgs: { limit, token },
							functionResolvedVariable: {
								currentSize: tokenCache.size,
								sizeMax: tokenCache.max,
								isTokenMax
							},
		...
		const currentTtlMsg =
			currentTtl >= 60
				? `${Math.floor(currentTtl / 60)} menit`
				: `${currentTtl} detik`;
		const isRateLimited = currentUsage >= limit;

		return isRateLimited
			? Promise.reject(
					rateLimitError.maximum_usage(
						...
						undefined,
						{
							...
							stack: null,
							functionDetails:
								'Ratelimit.check at utils/server_side.js line 173',
							functionArgs: { limit, token },
							functionResolvedVariable: {
								currentTtl,
								currentUsage,
...
export async function validateJWT(token, userId, otherOptions = {}) {
	...
	try {
		const algorithms = process.env.JWT_ALGORITHM
			? process.env.JWT_ALGORITHM.split(',')
			: ['HS256'];
		const issuer = process.env.JWT_ISSUER
			? process.env.JWT_ISSUER.split(',')
			: ['https://mbdyljoaqssmjfrumdad.supabase.co/auth/v1'];
...
export async function validateIdentifier(id, stamp, identifier) {
	try {
		...
		if (!isNumeric(stamp)) {
			throw new Error('Invalid stamp type');
		}
		...
		if (hmacDigestStr !== identifier) {
			throw new Error('Identifier not match');
		}
	} catch (error) {
		console.error(error);
		throw error;
...
export async function getEtag(data, algorithm = 'MD5', etagFormat = true) {
	try {
		if (!data) {
			throw new Error('Error generating Etag: No data provided');
		}
		const hmacFunction =
			CryptoJS[`Hmac${algorithm.toUpperCase()}`] || CryptoJS.HmacMD5;
...
export async function getSipkCookies(request = null) {
	...
	const allCookies = {
		serviceGuestCookie: request
			? cookiesRequest.get('s_guest_id')?.value
			: cookiesStore.get('s_guest_id')?.value,
		serviceUserIdCookie: request
			? cookiesRequest.get('s_user_id')?.value
			: cookiesStore.get('s_user_id')?.value,
		serviceAccessTokenCookie: request
			? cookiesRequest.get('s_access_token')?.value
			: cookiesStore.get('s_access_token')?.value,
		secureSessionCookie: request
			? cookiesRequest.get(process.env.USER_SESSION_COOKIES_NAME)?.value
			: cookiesStore.get(process.env.USER_SESSION_COOKIES_NAME)?.value
	};
...
export async function getApiKey(request) {
	const apiKey =
		headers().get('X-Api-Key') ?? request.nextUrl.searchParams.get('apiKey');
...
export async function getRequestDetails(
	filterSensitiveCookies = true,
	filterSensitiveHeaders = true
) {
	const ip = await getIpFromHeaders();
	const allCookies = cookies().getAll();
	const filteredCookies = filterSensitiveCookies
		? allCookies.filter(
				(cookie) =>
					!['_secure-auth.session-token', 's_access_token'].includes(
	...
	const filteredHeaders = filterSensitiveHeaders
		? Object.fromEntries(
				Object.entries(allHeaders).filter(
					([key]) =>
						!['authorization', 'x-api-key', 'cookie'].includes(
							key.toLowerCase()
						)
				)
			)
 ...
 */
export async function resetSipkCookies(
	cookiesName = [
		'secureSessionCookie',
		'serviceUserIdCookie',
	...
	if (cookiesName.includes('secureSessionCookie'))
		cookieStore.set({
			name: process.env.USER_SESSION_COOKIES_NAME,
			value: '',
			...cookieAuthDeleteOptions
		});
	if (cookiesName.includes('serviceUserIdCookie'))
		cookieStore.set({
			name: 's_user_id',
			value: '',
			...cookieServiceDeleteOptions
		});
	if (cookiesName.includes('serviceAccessTokenCookie'))
		cookieStore.set({
			name: 's_access_token',
			value: '',
			...cookieServiceDeleteOptions
		});
	if (cookiesName.includes('serviceGuestCookie'))
		cookieStore.set({
			name: 's_guest_id',
			value: '',
			...cookieServiceDeleteOptions
