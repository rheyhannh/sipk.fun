...
export const unixToDate = (
	unix,
	intlLocales = null,
	intlFormatOptions = null
) => {
	const date = new Date(unix * 1000);
	return date.toLocaleString(
		intlLocales || 'id-ID',
		intlFormatOptions || { dateStyle: 'full' }
	);
};
...
export const getLoadingMessage = (funWord = false, index = true) => {
	const message = {
		fun: ['Memproses keajaiban', 'Kunci adalah sabar', 'Membaca pikiranmu'],
		default: [
			'Memproses',
	...
	const usedIndex =
		index === true
			? Math.floor(Math.random() * maximumIndex)
			: index === 0
				? 0
				: index;
	return funWord
		? message.fun[usedIndex] || 'Memproses Keajaiban'
		: message.default[usedIndex] || 'Memproses permintaanmu';
};
...
export const getSessionTable = () => {
	...
	if (savedState) {
		try {
			...
			if (
				'tab' in state &&
				typeof state.tab === 'number' &&
				'pageSize' in state &&
				typeof state.pageSize === 'number' &&
				'pageIndex' in state &&
				typeof state.pageIndex === 'number' &&
				'pageControlPosition' in state &&
				typeof state.pageControlPosition === 'number' &&
				...
				'columnFilters' in state &&
				Array.isArray(state.columnFilters) &&
				'columnSorting' in state &&
				Array.isArray(state.columnSorting) &&
				'rowAction' in state &&
				typeof state.rowAction === 'boolean'
			) {
				const {
					tab,
					pageSize,
...
export const getSessionGrafik = () => {
	...
	if (savedState) {
		try {
			...
			if (
				'hideIp' in state &&
				typeof state.hideIp === 'boolean' &&
				'hideMatkul' in state &&
				typeof state.hideMatkul === 'boolean' &&
				'hideSks' in state &&
				typeof state.hideSks === 'boolean'
			) {
				...
				throw new Error('Invalid grafik state');
			}
		} catch (error) {
	...
	if (savedState) {
		try {
			...
			if (
				'tab' in state &&
				typeof state.tab === 'number' &&
				'swiperIndex' in state &&
				typeof state.swiperIndex === 'number'
			) {
	...
	if (savedState) {
		try {
			...
			if (
				...
				throw new Error('Invalid distribusi state');
			}
		} catch (error) {
...
export const checkStrongPassword = (password) => {
	if (!password) {
		return { point: -1, message: 'Password kosong', level: 'empty' };
	}
...
export const fetchWithAuth = async (
	...
	if (
		endpoint === 'custom' &&
		!customEndpoint &&
		typeof customEndpoint !== 'string'
	) {
		throw new TypeError('Custom endpoint tidak valid');
	}

	const target =
		endpoint === 'custom' ? customEndpoint : endpointByKey[endpoint];
	const baseUrl =
