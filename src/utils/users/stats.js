...
export const getUserIpk = (matkul) => {
	if (Array.isArray(matkul) && matkul.length !== 0) {
		...
		const { totalNilaiAkhir } = matkul.reduce(
			(sum, current) => {
				return {
...
export const getUserIpkPercentage = (user, matkul) => {
	if (
		Array.isArray(user) &&
		user.length !== 0 &&
		Array.isArray(matkul) &&
		matkul.length !== 0
	) {
		const ipk = parseFloat(getUserIpk(matkul));
		const ipkTarget = user[0]?.ipk_target;
		const ipkPercentage = ipkTarget
			? Math.round((ipk / ipkTarget) * 100)
			: Math.round((ipk / 4) * 100);
...
export const getUserSksPercentage = (user, matkul) => {
	if (
		...
		const sksPercentage = sksTarget
			? Math.round((sks / sksTarget) * 100)
			: Math.round((sks / 144) * 100);
...
export const getUserMatkulPercentage = (user, matkul) => {
	if (
		...
		const matkulPercentage = matkulTarget
			? Math.round((matkulTotal / matkulTarget) * 100)
			: Math.round((matkulTotal / 50) * 100);
...
export const getStatsSemester = (matkul, sort = false) => {
	if (Array.isArray(matkul) && matkul.length !== 0) {
		...
		return sort
			? statsSemester.sort((a, b) => a.semester - b.semester)
			: statsSemester;
	}
	...
	if (Array.isArray(matkul) && matkul.length !== 0) {
		...
		const result = Object.values(groupBySemester).map((group) => {
			...
			const on = group.filter(
				(item) => item.nilai.bobot >= item.target_nilai.bobot
			);
			const off = group.filter(
				(item) => item.nilai.bobot < item.target_nilai.bobot
			);
			const on_target = {
				matkul: on.length,
				sks: on.reduce((sum, item) => sum + item.sks, 0)
			};
			const off_target = {
				matkul: off.length,
				sks: off.reduce((sum, item) => sum + item.sks, 0)
			};
	...
	if (Array.isArray(matkul) && matkul.length !== 0) {
		...
		matkul.forEach((matakuliah) => {
			...
			Object.keys(penilaian).forEach((indeksNilai) => {
				const nilai = result[semesterKey].find(
					(item) => item.nilai === indeksNilai
				);
				const semua = result['semua'].find(
					(item) => item.nilai === indeksNilai
				);
				...
				if (nilai) {
					...
					nilai.sks +=
						matakuliah.nilai.indeks === indeksNilai ? matakuliah.sks : 0;
				} else {
