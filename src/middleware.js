...
import { encryptAES, decryptAES, getCookieOptions } from '@/utils/server_side';
...
export default async function middleware(request) {
	...
	const secureAuthSessionToken = request.cookies.get(
		`${process.env.USER_SESSION_COOKIES_NAME}`
	)?.value;
	...
	if (!secureAuthSessionToken) {
		...
		if (pathname === '/dashboard/matakuliah') {
			loginUrl.searchParams.set('from', 'matakuliah');
		}
		response = NextResponse.redirect(loginUrl);
		return response;
	}
			...
			cookies: {
				async get(name) {
					const decryptedSession =
						(await decryptAES(secureAuthSessionToken)) || 'removeMe';
