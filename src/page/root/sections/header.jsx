...
import { HEADER_NAVIGATION_SHORCUTS } from '../config';
// #endregion
...
const ButtonCTA = React.forwardRef(
	({ text = 'Lorem', onClick, href, ...props }, ref) => (
		<motion.a
			ref={ref}
	...
	return (
		<motion.div
			...
			variants={{
				hide: {
					y: -125,
					backgroundColor: showNavbarOverlay
						? 'var(--root-navbar-bgColorStrong)'
						: 'var(--root-navbar-bgColor)',
					backdropFilter: showNavbarOverlay
						? 'none'
						: 'blur(7.5px) saturate(50%)'
				},
				show: {
					y: 0,
					backgroundColor: showNavbarOverlay
...
const Header = () => {
	...
	const handleScrollDown = () => {
		...
		if (!showNavbarOverlay && scrollFrames.current > 5 && viewportWidth < 768)
			setShowNavbar(false);
	};

	const scrollingType = useScrollingEvent(
		handleScrollUp,
		handleScrollDown,
		null,
		{
