...
const ScrollingItem = ({ item, isActive, ...props }) => (
	<div className={`${styles.item} ${isActive ? styles.active : ''}`} {...props}>
		<div className={styles.logo}>{getLogoUniversitasByShort(item.short)}</div>
		<h3 className={styles.title_small}>{item.nama}</h3>
	</div>
);
...
const Universitas = ({ universitas }) => {
	...
	const replacedDescription = replacePlaceholders(
		UNIVERSITAS_SECTION_DESCRIPTION,
		{
				...
				<div className={styles.layout}>
					<div className={styles.text}>
						<AnimatedElement
							...
							scrollProgress={
								viewportWidth < 1024 ? null : sectionScrollProgress
							}
						...
						>
							...
							>
								...
								>
									...
									<FiArrowRight className={styles.arrow_right_icon} />
								</ButtonSimpleForwarded>
							</Link>
					...
					<div className={styles.content}>
						{iconAnimProps.length > 0 &&
							UNIVERSITAS_ITEMS_LAYOUT.map((config, layoutIndex) => {
								const currentItems = universitas.slice(
									itemIndex,
									...
									if (config?.style && Object.keys(config.style).length > 0) {
										Object.entries(config.style).forEach(([key, val]) => {
											if (typeof val === 'function')
												style[key] = val(viewportWidth, viewportHeight);
											else style[key] = val;
										});
									}
