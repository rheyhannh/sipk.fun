...
const HERO_TITLE_STAGGERED = shuffleArray(
	HERO_TITLE_PARAGRAPH.flat().map((_, index) => index * HERO_TITLE_DELAY_OFFSET)
);
						...
						<DashboardMockup
							...
							summary={[
								...
								},
								{
									...
									data: { value: 33, percentage: 66, keterangan: 'Targetmu 50' }
								},
								{
							...
							history={[
								{
									item: MatkulDummies[1],
									color: MatkulDummiesNilaiColorPreset[MatkulDummies[1].nilai]
								},
								{
									item: MatkulDummies[0],
									color: MatkulDummiesNilaiColorPreset[MatkulDummies[0].nilai]
								},
								{
									item: MatkulDummies[4],
									color: MatkulDummiesNilaiColorPreset[MatkulDummies[4].nilai]
								}
							]}
						/>
					</Mockup3D>
 ...
 * @returns {React.ReactElement} Rendered component
 */
function Title({ id = 'title', label = 'Awesome Title' }) {
	return (
		<VisibilityWrapper
			as='h1'
			id={id}
		...
		>
			{HERO_TITLE_PARAGRAPH.flat().map((text, index) => (
				<HighlightText key={index} text={text} {...resolveTitleProps(text)} />
			))}
		</VisibilityWrapper>
	);
}
	...
	return (
		<VisibilityWrapper>
			...
			>
				...
				>
					<span>{item[1]?.text || 'Click Me'}</span>
					<span className={styles.icon}>
						<FiArrowDown />
					</span>
				</ButtonSimple>
...
function MockupContainer({ children }) {
	return <div className={styles.mockup_container}>{children}</div>;
}
...
function MockupPerspective({ children }) {
	return <div className={styles.mockup_perspective}>{children}</div>;
}
...
function Mockup3D({ children }) {
	return <div className={styles.mockup_3d}>{children}</div>;
}
...
function resolveTitleProps(text) {
	return {
		...
		adjustWavingFlyIn: {
			baseDelay:
				HERO_TITLE_STAGGERED[
					findArrayIndexByString(text, HERO_TITLE_PARAGRAPH.flat())
				],
			y: [-125, 0],
			duration: 1.75
		}
