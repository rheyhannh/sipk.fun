	...
	return (
		<div className={`${styles.wrapper} ${getClassnameByState()}`} {...props}>
			<div className={styles.border__wrapper}>
				<span
					className={`${styles.border__top} ${states.success || states.error || !states.loading ? styles.hide : ''}`}
				/>
				<span
					className={`${styles.border__right} ${states.success || states.error || !states.loading ? styles.hide : ''}`}
				/>
				<span
					className={`${styles.border__bottom} ${states.success || states.error || !states.loading ? styles.hide : ''}`}
				/>
				<span
					className={`${styles.border__left} ${states.success || states.error || !states.loading ? styles.hide : ''}`}
				/>
			</div>
			<div className={`${styles.icon__outter} ${getClassnameByState()}`} />
			<div className={`${styles.icon__inner} ${getClassnameByState()}`}>
				<div
					className={`${styles.icon} ${styles.loading} ${states.loading ? styles.active : ''} ${states.success || states.error ? styles.hide : ''}`}
				>
					<FaLink />
				</div>
				<div
					className={`${styles.icon} ${styles.success} ${states.success ? styles.active : ''}`}
				>
					<FaCheck />
				</div>
				<div
					className={`${styles.icon} ${styles.error} ${states.error ? styles.active : ''}`}
				>
					<FaExclamation />
				</div>
			</div>
