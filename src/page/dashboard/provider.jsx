		...
		<ErrorBoundary
			...
			onError={(error, info) =>
				handleReactErrorBoundary(error, info, cookies, {
					boundaryLocation: 'DashboardProvider'
				})
			}
		>
...
function DashboardRootError({ error, resetErrorBoundary }) {
	const handleReset = async () => {
		const toastId = toast.loading('Memulai prosedur moveon', {
			position: 'top-center'
		});
		const { pathname } = window.location;
		const fromParam =
			pathname === '/dashboard/matakuliah' ? 'matakuliah' : 'dashboard';
		...
		<div className={styles.root_error_container}>
			<div className={styles.root_error_content}>
				...
				<div className={styles.text}>
					...
					<p>
						Sepertinya terjadi kesalahan tak terduga. Kamu bisa coba reset SIPK
						lalu login ulang dengan klik tombol dibawah. Kalau masalah ini masih
						muncul setelah login ulang, kayaknya bakal ada yang lembur buat
						benerin ini 😞
					</p>
				</div>
