...
export default async function getNotifikasi(sort = 'desc') {
	const response = await fetch(process.env.SUPABASE_API_URL + '/notifikasi', {
		method: 'GET',
		headers: {
	...
	if (sort === 'asc')
		return data.sort((a, b) => a.unix_created_at - b.unix_created_at);
	else if (sort === 'desc')
		return data.sort((a, b) => b.unix_created_at - a.unix_created_at);
	else return data;
}
