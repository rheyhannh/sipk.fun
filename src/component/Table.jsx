...
export function Table({
	...
	const SkeletonTable = () => {
		const tableLoadingImg = (
			<Image
				src='/table_loading.svg'
				width={100}
				height={100}
				alt='Table Loading'
			/>
				...
				<div className={styles.tools}>
					<div className={`${styles.tools__tabs} ${styles.skeleton}`}>
						{['134px', '70px', '55px'].map((item, index) => (
							<div
								style={{ minWidth: item }}
								className={`${styles.btn} ${styles.skeleton}`}
								key={item}
							>
								<SkeletonTheme
	...
	const LoadedTable = () => {
		...
		const [columnVisibility, setColumnVisibility] = useState(
			sessionTable?.columnVisibility ??
				user?.preferences?.table?.columnVisibility ?? {
					nomor: true,
					matakuliah: true,
					semester: true,
					sks: true,
		...
		const [columnFilters, setColumnFilters] = useState(
			sessionTable?.columnFilters ?? []
		);
		const [columnOrder, setColumnOrder] = useState(
			sessionTable?.columnOrder ??
				user?.preferences?.table?.columnOrder ?? [
					'nomor',
					'matakuliah',
					'semester',
					'sks',
		...
		const [pageControlPosition, setPageControlPosition] = useState(
			sessionTable?.pageControlPosition ??
				user?.preferences?.table?.controlPosition ??
				0
		);
		const [rowAction, setRowAction] = useState(sessionTable?.rowAction ?? true);
		const [pagesIndex, setPagesIndex] = useState(
			sessionTable?.pageIndex ? sessionTable.pageIndex + 1 : 1
		);
		...
		const columns = useMemo(
			() => [
				...
				columnHelper.accessor((row) => row.dapat_diulang, {
					...
					cell: (info) =>
						info.getValue() ? (
							<IoCheckmark size={'15px'} color={'var(--success-color)'} />
						) : (
							<IoClose size={'15px'} color={'var(--danger-color)'} />
						),
				...
				columnHelper.accessor(
					(row) => row.nilai.bobot >= row.target_nilai.bobot,
					{
						id: 'ontarget',
						cell: (info) =>
							info.getValue() ? (
		...
		const getTablePreferences = () => {
			...
			return {
				size: currentPageSize === matkul.length + 1 ? -1 : currentPageSize,
				controlPosition: pageControlPosition,
				state: columnState
			};
		};
		...
		return (
			<div
				className={`${styles.container} ${pageControlCSS[pageControlPosition]}`}
			>
				<div
					className={`${styles.validating} ${isValidating ? '' : styles.hide}`}
				>
				...
				<div className={styles.tools}>
					<div className={styles.tools__tabs}>
						...
						<div
							...
							onClick={
								activeTab !== 1 ? () => handleDeletedMatakuliahTab() : null
							}
						>
							Dihapus
						</div>
						<div
							...
							onClick={
								activeTab !== 2 ? () => handleEditedMatakuliahTab() : null
							}
						>
							Diedit
						</div>
					...
					<div className={styles.tools__right}>
						<div className={styles.tools__search}>
							<CariMatakuliah
								initValue={table.getColumn('matakuliah').getFilterValue()}
								column={table.getColumn('matakuliah')}
								table={table}
								isSearchActive={isSearchActive}
								setPagesIndex={setPagesIndex}
							/>
						</div>
						<div className={styles.tools__shorcut}>
							<div
								className={styles.tools__shorcut_box}
								onClick={() => {
									handleTambahModal();
								}}
							>
							...
							<div
								className={`${styles.tools__shorcut_box} ${isTableFiltered() ? styles.active : ''}`}
								onClick={() => {
									handleFilterModal();
								}}
							>
						...
						<div className={styles.pagination}>
							<div className={styles.pagination__pages}>
								<div className={styles.pagination__pages_number}>
									...
									>
										{[5, 10, 25, 50, 100, matkul.length + 1].map(
											(pageSize, index) => (
									...
									onClick={
										table.getCanNextPage()
											? () => {
													...
													setPagesIndex(table.getPageCount());
												}
											: null
									}
									className={`${styles.pagination__navs} ${!table.getCanNextPage() ? styles.disabled : ''}`}
													...
													{header.isPlaceholder ? null : (
														<div>
															...
															>
																...
																{{
																	asc:
																		header.column.id === 'diulang' ||
																		header.column.id === 'ontarget' ? (
																			<IoCheckmark />
																		) : (
																			<IoArrowDownSharp />
																		),
										...
										<tr
											onClick={(e) => {
												if (!e.target.closest(`.${styles.action}`)) {
													if (activeTab === 0) {
														const createdAt = getCreatedAtById(
															row.getValue('nomor')
														);
														const updatedAt = getUpdatedAtById(
															row.getValue('nomor')
														);
														const itemData = {
															...
															dibuat: createdAt
																? unixToDate(createdAt, null, {
																		dateStyle: 'full',
																		timeStyle: 'medium'
																	})
																: '-',
															diedit: updatedAt
																? unixToDate(updatedAt, null, {
																		dateStyle: 'full',
																		timeStyle: 'medium'
																	})
																: '-'
															...
															{...(isNilaiCell || isTargetCell
																? {
																		style: {
																			...
																			minWidth: '28px',
																			minHeight: '28px',
																			background: nilaiColor
																				? `var(--box-color-${nilaiColor})`
																				: 'var(--box-color-bg2)',
																			borderRadius: '50%'
																		}
																	}
	...
	if (state === 'loading') {
		...
		return 'Unidentified Table State';
	}
}

function CariMatakuliah({ column, table, isSearchActive, setPagesIndex }) {
			...
			<div className={styles.wrapper}>
				...
				{activeTab === 1 || activeTab === 2 ? (
					<>
						<i
							onClick={() => {
								handleUndoConfirmModal(row.getValue('nomor'));
							}}
						>
							<FaUndo size={'13px'} />
						</i>
