		...
		<div
			...
			style={{
				...
				'--field-backgroundColor':
					fieldOptions?.backgroundColor || 'var(--light-color)',
				'--field-margin': fieldOptions?.margin || '0.5rem 0',
				'--field-borderRadius': fieldOptions?.borderRadius || '50px',
				'--field-gridTemplateColumns':
					fieldOptions?.gridTemplateColumns || '1fr 70% 1fr',
				'--icon-color': iconColor || 'var(--infoDark-color)',
				'--validator-emptyColor':
					validatorIconColor?.empty || DEFAULT_FANCYINPUT_ICONCOLOR['empty'],
				'--validator-validatingColor':
					validatorIconColor?.validating ||
					DEFAULT_FANCYINPUT_ICONCOLOR['validating'],
				'--validator-successColor':
					validatorIconColor?.success ||
					DEFAULT_FANCYINPUT_ICONCOLOR['success'],
				'--validator-warningColor':
					validatorIconColor?.warning ||
					DEFAULT_FANCYINPUT_ICONCOLOR['warning'],
				'--validator-dangerColor':
					validatorIconColor?.danger || DEFAULT_FANCYINPUT_ICONCOLOR['danger'],
				'--validator-errorColor':
					validatorIconColor?.error || DEFAULT_FANCYINPUT_ICONCOLOR['error'],
				'--input-padding': inputOptions?.padding || '0 0.25rem',
				'--input-color': inputOptions?.color || 'var(--dark-color)',
				'--input-fontWeight':
					inputOptions?.fontWeight || 'var(--font-semi-bold)',
				'--placeholder-color':
					placeholderOptions?.color || 'var(--infoDark-color)',
				'--placeholder-fontWeight':
					placeholderOptions?.fontWeight || 'var(--font-medium)'
			}}
		>
			<i>{icon}</i>
			...
			<i className={`${styles.validator} ${styles[state]}`}>
				<span onClick={() => validatorOnClick?.(state, msg)}>{vIcon}</span>
			</i>
		</div>
	);
}
