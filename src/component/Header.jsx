...
function Right() {
	...
	const { setModal, setActive, setData } = useContext(ModalContext);

	const userIdCookie = useCookies().get('s_user_id');

	const helloDark = () =>
		toast('Dark Mode', { duration: 2000, icon: <FiMoon size={'17px'} /> });
	const helloLight = () =>
		toast('Light Mode', { duration: 2000, icon: <FiSun size={'17px'} /> });
	...
	if (error) {
		return (
			<>
				...
				<div
					className={`${styles.dashboard__profile} ${styles.error}`}
					onClick={handleRetry}
				>
		...
		return (
			<>
				<div
					...
					className={styles.dashboard__theme}
				>
					<span
						className={theme === 'light' ? styles.active : ''}
						onClick={() => {
				...
				<div className={styles.dashboard__profile}>
					<div
						className={`${styles.dashboard__profile_info} ${styles.skeleton}`}
					>
						<Skeleton
							width={'100%'}
							height={'100%'}
					...
					<div
						className={`${styles.dashboard__profile_avatar} ${styles.skeleton}`}
					>
						<Skeleton
							borderRadius={'50%'}
							width={'100%'}
	...
	return (
		<>
			...
			<div
				className={styles.dashboard__profile}
				onClick={() => {
					handleProfilModal();
				}}
			>
				<div className={`${styles.dashboard__profile_info}`}>
					<p>
						<b data-cy={'header_profile_fullname'}>{data[0].fullname}</b>
					</p>
					<small data-cy={'header_profile_nickname'}>{data[0].nickname}</small>
				</div>
...
export default function Header() {
	...
	const { isNavbarActive, setNavbarActive, isPhoneContent, isRichContent } =
	...
	return (
		<>
			<div
				className={`${styles.dashboard} ${!showHeader ? styles.hide : ''}`}
				id='header'
			>
				<div
					onClick={handleNavbarClick}
					className={`${styles.hamburger} ${isNavbarActive ? styles.active : ''}`}
				>
