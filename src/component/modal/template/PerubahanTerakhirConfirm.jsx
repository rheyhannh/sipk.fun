...
const PerubahanTerakhirConfirm = () => {
	...
	return (
		<ModalContext.Consumer>
			{
				/** @param {import('@/types/context').ModalContext<PerubahanTerakhirConfirmModalData>} context */ (
					context
				) => {
					const { type, nama } = context?.data?.current ?? {
						type: '',
						nama: ''
					};
					...
					const renderTitle = () => {
						if (type === 'tambah') {
							return `Hapus Matakuliah`;
						} else if (type === 'hapus') {
							return `Tambah Matakuliah`;
						} else if (type === 'ubah') {
							return `Ubah Matakuliah`;
						} else {
						...
						if (type === 'tambah') {
							return (
								<p>
									...
									<b style={{ fontWeight: '600' }}>{nama}</b> yang sudah
									ditambah?
								</p>
							);
						} else if (type === 'hapus') {
							...
							return (
								<p>
									Kamu ingin mengubah{' '}
									<b style={{ fontWeight: '600' }}>{nama}</b> ke data
									sebelumnya?
								</p>
							);
						} else {
							return <p>Sepertinya ada yang salah</p>;
						}
					};
								...
								return new Promise(async (resolve, reject) => {
									try {
										...
										var {
											stamp: _,
											type: _,
											nilai: { indeks: indeksNilai },
											target_nilai: { indeks: indeksTarget },
											...prevData
										} = context.data.prev;
										const prevDataFiltered = {
											...prevData,
											nilai: { indeks: indeksNilai },
											target_nilai: { indeks: indeksTarget }
										};
					...
					return (
						<Backdrop>
							<Layout
								className={`${styles.perubahan__terakhir} ${styles.confirm}`}
							>
