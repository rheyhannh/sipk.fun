...
const FormTabs = ({ isSebelumForm, setIsSebelumForm, ...props }) => (
	<div className={styles.form__type} {...props}>
		<span
			className={isSebelumForm ? styles.active : ''}
			onClick={() => {
				setIsSebelumForm(true);
			}}
		>
			<h3>Sebelum</h3>
		</span>
...
const PerubahanTerakhirDetail = () => {
	...
	return (
		<ModalContext.Consumer>
			{
				/** @param {import('@/types/context').ModalContext<PerubahanTerakhirDetailModalData>} context */ (
					context
				) => {
									...
									if (!response.ok) {
										...
										if (
											...
											handleApiErrorResponse(
												'DELETE',
												getApiURL('matkul', { id: context.data.matkul_id }),
												parsed,
												cookies
											);
										}
												...
												populateCache: (_, currentMatkul) => {
													if (!currentMatkul) {
														...
														const filteredMatkul = currentMatkul.filter(
															(matkul) =>
																matkul.id !== `${context.data.matkul_id}`
														);
						...
						<Backdrop>
							{context?.data?.current?.type === 'ubah' ? (
								...
								>
									...
									<FormTabs
										isSebelumForm={isSebelumForm}
										setIsSebelumForm={setIsSebelumForm}
									/>
									<Inner>
										<form>
											...
											<div className={styles.gtc_repeat3_1fr}>
												<div className={styles.form__input_field}>
													<input
														...
														value={
															isSebelumForm
																? context?.data?.prev?.sks || '-1'
																: context?.data?.current?.sks || '-1'
														}
														autoComplete='off'
														disabled
													/>
												...
												<div className={styles.form__input_field}>
													<input
														...
														value={
															isSebelumForm
																? context?.data?.prev?.nilai?.indeks || '-'
																: context?.data?.current?.nilai?.indeks || '-'
														}
														autoComplete='off'
														disabled
													/>
												...
												<div className={styles.form__input_field}>
													<input
														...
														value={
															isSebelumForm
																? context?.data?.prev?.semester || '-1'
																: context?.data?.current?.semester || '-1'
														}
														autoComplete='off'
														disabled
													/>
											...
											<div className={styles.gtc_repeat2_1fr}>
												<div className={styles.form__input_field}>
													<input
														...
														value={
															isSebelumForm
																? context?.data?.prev?.dapat_diulang
																	? 'Ya'
																	: 'Tidak'
																: context?.data?.current?.dapat_diulang
																	? 'Ya'
																	: 'Tidak'
														}
														autoComplete='off'
												...
												<div className={styles.form__input_field}>
													<input
														...
														value={
															isSebelumForm
																? context?.data?.prev?.target_nilai?.indeks ||
																	'-'
																: context?.data?.current?.target_nilai
																		?.indeks || '-'
														}
														autoComplete='off'
														disabled
											...
											<div className={styles.form__input_field}>
												<input
													...
													value={
														isSebelumForm
															? context?.data?.prev?.stamp
																? unixToDate(context?.data?.prev?.stamp, null, {
																		dateStyle: 'full',
																		timeStyle: 'medium'
																	})
																: 'Tidak Diketahui'
															: context?.data?.current?.stamp
																? unixToDate(
																		context?.data?.current?.stamp,
																		null,
												...
												<label htmlFor='date' className={styles.form__label}>
													{isSebelumForm
														? context?.data?.prev?.type === 'tambah'
															? 'Ditambah'
															: context?.data?.prev?.type === 'ubah'
																? 'Diedit'
																: context?.data?.prev?.type === 'hapus'
																	? 'Dihapus'
																	: 'Tanggal'
														: context?.data?.current?.type === 'tambah'
															? 'Ditambah'
															: context?.data?.current?.type === 'ubah'
																? 'Diedit'
																: context?.data?.current?.type === 'hapus'
																	? 'Dihapus'
																	: 'Tanggal'}
												</label>
											</div>
											...
											<div className={styles.gtc_repeat2_1fr}>
												...
												<div className={styles.form__input_field}>
													<input
														...
														className={styles.form__input}
														value={
															context?.data?.current?.target_nilai?.indeks ||
															'-'
														}
														autoComplete='off'
														disabled
