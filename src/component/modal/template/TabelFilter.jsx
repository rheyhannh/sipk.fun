...
import {
	...
	Button,
	Inner
} from '@/component/modal/components';
// #endregion
...
const TabelFilter = () => {
	...
	return (
		<ModalContext.Consumer>
			{
				/** @param {import('@/types/context').ModalContext<TabelFilterModalData>} context */ (
					context
				) => {
					...
					const getBooleanFilteredNilai = (filter) => {
						...
						return penilaianKey.map((value) =>
							filter.includes(value.toUpperCase())
						);
					};
					...
					const validateForm = () => {
						return new Promise(async (resolve, reject) => {
							try {
								const allValue = [-1, '-1'];
								if (nama) {
									if (!isLength(nama, { max: 50 })) {
										setErrorMessage('Nama matakuliah maksimal 50 karakter');
										resolve(null);
									}
								}
								if (sksMin && sksMaks) {
									if (!isInt(sksMin, { min: 0, max: 50 })) {
										setErrorMessage('Sks minimal harus angka 0 - 50');
										resolve(null);
									}
									if (!isInt(sksMaks, { min: 0, max: 50 })) {
										setErrorMessage('Sks maksimal harus angka 0 - 50');
										resolve(null);
									}
									if (sksMin > sksMaks) {
										setErrorMessage(
											'Sks maksimal harus lebih besar dari sks minimal'
										);
										resolve(null);
									}
								}
								...
								if (semesterMin && semesterMaks) {
									if (!isInt(semesterMin, { min: 0, max: 50 })) {
										setErrorMessage('Semester minimal harus angka 0 - 50');
										resolve(null);
									}
									if (!isInt(semesterMaks, { min: 0, max: 50 })) {
										setErrorMessage('Semester maksimal harus angka 0 - 50');
										resolve(null);
									}
									if (semesterMin > semesterMaks) {
										setErrorMessage(
											'Semester maksimal harus lebih besar dari minimal semester'
										);
										resolve(null);
									}
								}
								...
								const filters = [];
								const setNilaiFilters = penilaianKey.filter(
									(_, index) => filteredNilai[index]
								);
								const setTargetNilaiFilters = penilaianKey.filter(
									(_, index) => filteredTargetNilai[index]
								);
								if (nama) {
									filters.push({ id: 'matakuliah', value: nama });
								}
								if (sksMin || sksMaks) {
									filters.push({ id: 'sks', value: [sksMin, sksMaks] });
								}
								if (semesterMin || semesterMaks) {
									filters.push({
										id: 'semester',
										value: [semesterMin, semesterMaks]
									});
								}
								if (setNilaiFilters.length) {
									filters.push({ id: 'nilai', value: setNilaiFilters });
								}
								if (setTargetNilaiFilters.length) {
									filters.push({ id: 'target', value: setTargetNilaiFilters });
								}
								if (dapatDiulang && !allValue.includes(dapatDiulang)) {
									filters.push({
										id: 'diulang',
										value: dapatDiulang === 'ya' ? true : false
									});
								}
								if (onTarget && !allValue.includes(onTarget)) {
									filters.push({
										id: 'ontarget',
										value: onTarget === 'ya' ? true : false
									});
								}
								resolve(filters);
							} catch (error) {
					...
					const toggleEditFilter = () => {
						if (editFilter) {
							...
							setFilteredNilai(
								getBooleanFilteredNilai(context?.data?.currentFilters?.nilai)
							);
							setFilteredTargetNilai(
								getBooleanFilteredNilai(context?.data?.currentFilters?.target)
							);
					...
					return (
						<Backdrop>
							<Layout
								...
								onEsc={true}
								onEnter={(e, isLayout) => {
									if (isLayout && editFilter) handleApplyFilter(e);
								}}
								style={
									editFilter
										? {
							...
							>
								...
								<Inner>
											...
											<div>
												<input
													...
													value={
														editFilter
															? nama
															: context?.data?.currentFilters?.matakuliah || ''
													}
													...
													disabled={!editFilter}
												/>
												<label htmlFor='nama' className={styles.form__label}>
													Nama
												</label>
											</div>

											<div
												className={`${styles.form__input_length} ${nama.length >= 50 ? styles.max : ''}`}
											>
												...
												<div className={styles.form__input_field}>
													<input
														...
														value={
															editFilter
																? sksMin
																: context?.data?.currentFilters?.sks[0] || ''
														}
														onChange={(e) => {
															setSksMin(e.target.value);
														}}
														onFocus={() => {
															setErrorMessage('');
														}}
												...
												<div className={styles.form__input_field}>
													<input
														...
														value={
															editFilter
																? sksMaks
																: context?.data?.currentFilters?.sks[1] || ''
														}
														onChange={(e) => {
															setSksMaks(e.target.value);
														}}
														onFocus={() => {
															setErrorMessage('');
														}}
												...
												<div className={styles.form__input_field}>
													<input
														...
														value={
															editFilter
																? semesterMin
																: context?.data?.currentFilters?.semester[0] ||
																	''
														}
														onChange={(e) => {
															setSemesterMin(e.target.value);
														}}
														onFocus={() => {
															setErrorMessage('');
														}}
												...
												<div className={styles.form__input_field}>
													<input
														...
														value={
															editFilter
																? semesterMaks
																: context?.data?.currentFilters?.semester[1] ||
																	''
														}
														onChange={(e) => {
															setSemesterMaks(e.target.value);
														}}
														onFocus={() => {
															setErrorMessage('');
														}}
										...
										<div className={styles.form__input_field}>
											<select
												...
												value={
													editFilter
														? dapatDiulang
														: context?.data?.currentFilters?.diulang || ''
												}
												onChange={(e) => {
													setDapatDiulang(e.target.value);
												}}
												onFocus={() => {
													setErrorMessage('');
												}}
										...
										<div className={styles.form__input_field}>
											<select
												...
												value={
													editFilter
														? onTarget
														: context?.data?.currentFilters?.ontarget || ''
												}
												onChange={(e) => {
													setOnTarget(e.target.value);
												}}
												onFocus={() => {
													setErrorMessage('');
												}}
											...
											<label htmlFor='onTarget' className={styles.form__label}>
												On Target
											</label>
										</div>
									</div>
									...
									<div
										...
										className={styles.ftmn}
									>
										<div>
											<h3
												style={{ marginBottom: '.5rem' }}
												className={styles.tabel__filter_title}
											>
												Nilai
											</h3>
														...
														<span
															...
															onClick={
																editFilter
																	? (event) => {
																			event.target.blur();
																			const updatedState = filteredNilai.map(
																				(item, i) =>
																					i === index ? !item : item
																			);
															...
															className={`${styles.item} ${styles[type]} ${filteredNilai[index] ? styles.active : ''}`}
															key={key}
														>
															{key}
														</span>
												...
												{penilaianKey.map((key, index) => {
													...
													return (
														<span
															...
															onClick={
																editFilter
																	? (event) => {
																			event.target.blur();
																			const updatedState =
																				filteredTargetNilai.map((item, i) =>
																					i === index ? !item : item
																				);
															...
															className={`${styles.item} ${styles[type]} ${filteredTargetNilai[index] ? styles.active : ''}`}
															key={key}
														>
															{key}
														</span>
								...
								{editFilter ? (
									<div className={styles.form__action}>
										<div
											className={styles.gtc_repeat2_1fr}
											style={{ height: '100%' }}
										>
											<Button
												title='Reset'
