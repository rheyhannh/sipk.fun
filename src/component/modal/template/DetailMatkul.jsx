...
const DetailMatkul = () => {
	...
	return (
		<ModalContext.Consumer>
			{
				/** @param {import('@/types/context').ModalContext<DetailMatkulModalData>} context */ (
					context
				) => {
						...
						return new Promise(async (resolve, reject) => {
							try {
								...
								if (isEmpty(nama, { ignore_whitespace: true })) {
									setErrorMessage('Nama matakuliah dibutuhkan');
									resolve(null);
								} else {
									if (!isLength(nama, { min: 3, max: 50 })) {
										setErrorMessage(
											'Nama matakuliah minimal 3 karakter maksimal 50 karakter'
										);
										resolve(null);
									}
								}
								...
								// Validating 'Sks'
								if (isEmpty(sks, { ignore_whitespace: true })) {
									setErrorMessage('Sks matakuliah dibutuhkan');
									resolve(null);
								} else {
									if (!isInt(sks, { min: 0, max: 50 })) {
										setErrorMessage('Sks harus angka (min: 0, max: 50)');
										resolve(null);
									}
								}

								// Validating 'Nilai'
								if (!penilaian.hasOwnProperty(nilai)) {
									setErrorMessage('Nilai matakuliah dibutuhkan');
									resolve(null);
								} else {
									if (Number(penilaian[nilai].weight) > 4) {
										setErrorMessage('Nilai tidak valid');
										resolve(null);
									}
								}

								// Validating 'Semester'
								if (isEmpty(semester, { ignore_whitespace: true })) {
									setErrorMessage('Semester matakuliah dibutuhkan');
									resolve(null);
								} else {
									if (!isInt(semester, { min: 0, max: 50 })) {
										setErrorMessage('Semester harus angka (min: 0, max: 50)');
										resolve(null);
									}
								}
									...
									if (!response.ok) {
										...
										if (
											...
											handleApiErrorResponse(
												'PATCH',
												getApiURL('matkul', { id: context.data.id }),
												parsed,
												cookies
											);
										}
											...
											mutate(['/api/matkul', userIdCookie], matkul, {
												populateCache: (matkul, currentMatkul) => {
													if (!currentMatkul || !currentMatkul.length) {
														...
														const findIndex = currentMatkul.findIndex(
															(item) => item.id === matkul.id
														);
														if (findIndex !== -1) {
															return currentMatkul.map((item, index) =>
																index === findIndex ? matkul : item
															);
														} else {
											...
											mutate(['/api/matkul-history', userIdCookie], ref, {
												populateCache: (ref, currentRef) => {
													if (!currentRef || !currentRef.length) {
														...
														const filteredRef = currentRef.filter(
															(item) => item.id !== ref.id
														);
											...
											mutate(['/api/matkul', userIdCookie], undefined, {
												populateCache: (_, currentMatkul) => {
													if (!currentMatkul) {
														...
														const filteredMatkul = currentMatkul.filter(
															(matkul) => matkul.id !== `${context.data.id}`
														);
					...
					return (
						<Backdrop>
							...
							>
								<Head
									title={ubahMatkul ? 'Ubah Matakuliah' : 'Detail Matakuliah'}
								/>
									...
									<div className={styles.gtc_repeat3_1fr}>
										<div className={styles.form__input_field}>
											<input
												...
												required
											/>
											<label htmlFor='sks' className={styles.form__label}>
												Sks
											</label>
										</div>
										<div className={styles.form__input_field}>
											...
											>
												...
												{penilaianKey.map((nilai, index) => (
													<option value={nilai} key={nilai}>
														{nilai}
													</option>
												))}
											</select>
											<label htmlFor='nilai' className={styles.form__label}>
												Nilai
											</label>
										</div>
										<div className={styles.form__input_field}>
											<input
												...
												className={styles.form__input}
												value={
													ubahMatkul ? semester : context?.data?.semester || ''
												}
												onChange={(e) => {
												...
												required
											/>
											<label htmlFor='semester' className={styles.form__label}>
												Semester
											</label>
										</div>
									</div>
									...
									<div className={styles.gtc_repeat2_1fr}>
										...
										<div className={styles.form__input_field}>
											<select
												...
												value={
													ubahMatkul ? targetNilai : context?.data?.target || ''
												}
												onChange={(e) => {
													setTargetNilai(e.target.value);
												}}
												onFocus={() => {
													setErrorMessage('');
												}}
											...
											<label htmlFor='maxNilai' className={styles.form__label}>
												Target Nilai
											</label>
										</div>
									</div>
									...
									>
										<div>
											<input
												...
												className={`${styles.form__input}`}
												style={{
													cursor: ubahMatkul ? 'not-allowed' : 'default'
												}}
												...
												required
											/>
											<label htmlFor='dibuat' className={styles.form__label}>
												Ditambah
											</label>
										</div>
									</div>
									...
									<div className={styles.form__input_field}>
										<div>
											<input
												...
												required
											/>
											<label htmlFor='diedit' className={styles.form__label}>
												Diedit
											</label>
										</div>
									</div>
