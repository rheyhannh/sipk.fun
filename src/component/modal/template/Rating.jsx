...
const Rating = () => {
	...
	const handleAuthorChange = (e) => {
		...
		if (['0', '1', '2'].includes(newAuthor)) {
			...
			if (newAuthor === '0') {
				setInfo('Rating tampil dengan nama lengkap kamu');
			} else if (newAuthor === '1') {
				setInfo('Rating tampil dengan nickname kamu');
			} else if (newAuthor === '2') {
				setInfo('Rating tampil sebagai anonim');
			} else {
				setInfo('');
			}
		}
				...
				/** @param {import('@/types/context').ModalContext<any>} context */ (
					...
					const toggleEditRating = () => {
						if (editRating) {
							...
							setAuthor(
								ratingData.length ? ratingData[0].details.authorType : 0
							);
							setEditRating(true);
						}
					};
					...
					const validateForm = () => {
						return new Promise(async (resolve, reject) => {
							try {
								...
								if (stars <= 0) {
									setErrorMessage('Kamu harus pilih bintang 1 - 5');
									resolve(null);
								}
								if (stars > 5) {
									setStars(5);
								...
								if (review.length > 200) {
									setErrorMessage('Pesan maksimal 200 karakter');
									resolve(null);
								}
								if (unallowedWords.some((word) => review.includes(word))) {
									setErrorMessage('Pesan tidak dapat mengandung URL');
									resolve(null);
								}
								if (
									unallowedSymbols.some((symbol) => review.includes(symbol))
								) {
									setErrorMessage(
										`Pesan tidak dapat mengandung simbol > , < , & , ' , " dan /`
									);
									return false;
								}
						...
						const editRating = () => {
							return new Promise(async (resolve, reject) => {
								try {
									...
									if (!accessToken || !userIdCookie || !ratingId) {
										router.refresh();
										throw new Error('Terjadi kesalahan, silahkan coba lagi');
									}
							...
							>
								...
								{ratingError ? (
									...
									>
										...
										<h1 style={{ cursor: 'pointer' }} onClick={handleRetry}>
											&#x21bb;
										</h1>
									</div>
									...
									<Inner>
										...
										{ratingData.length ? (
											<>
												...
												<textarea
													placeholder={
														editRating ? placeholderByStars[stars] : ''
													}
												...
												>
													<div
														className={`${styles.review} ${editRating ? (review.length >= 200 ? styles.max : '') : ratingData[0].review.length >= 200 ? styles.max : ''}`}
													>
														{editRating
															? info
																? info
																: 200 - review.length + ' karakter tersisa'
															: ''}
													</div>
													<select
														id='authorRating'
														className={
															disableBorder ? styles.border_off : undefined
														}
														value={
															editRating
																? author
																: ratingData[0].details.authorType
														}
											...
											<>
												...
												>
													<div
														className={`${styles.review} ${review.length >= 200 ? styles.max : ''}`}
													>
														{info
															? info
															: review.length > 0
								...
								{ratingLoading ||
								ratingValidating ||
								ratingError ? null : ratingData.length ? (
									<>
										<div
											style={
												editRating
