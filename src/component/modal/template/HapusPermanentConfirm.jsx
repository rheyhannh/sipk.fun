...
const HapusPermanentConfirm = () => {
	...
	return (
		<ModalContext.Consumer>
			{
				/** @param {import('@/types/context').ModalContext<HapusPermanentConfirmModalData>} context */ (
					context
				) => {
					const matakuliah = context?.data?.nama
						? context?.data?.nama
						...
						const deletePermanent = () => {
							return new Promise(async (resolve, reject) => {
								try {
									...
									const idParamsValue = context?.data?.fromTabel
										? context.data.id
										: context.data.refId;
									const midParamsValue = context?.data?.fromTabel
									...
									const response = await fetchWithAuth(
										...
										accessToken,
										null,
										{ id: idParamsValue, mid: midParamsValue }
									);
												...
												populateCache: (_, currentMatkul) => {
													if (!currentMatkul) {
														...
														const filteredMatkul = currentMatkul.filter(
															(matkul) => matkul.id !== `${midParamsValue}`
														);
							...
							>
								...
								<div style={{ color: 'var(--infoDark-color)' }}>
									<p>
										...
										{matakuliah ? (
											<b style={{ fontWeight: '600' }}> {matakuliah} </b>
										) : (
											' matakuliah ini '
										)}
