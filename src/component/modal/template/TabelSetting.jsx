...
const TabelSetting = () => {
	...
	const [disablePageSizeBorder, setDisablePageSizeBorder] =
	...
	return (
		<ModalContext.Consumer>
			{
				/** @param {import('@/types/context').ModalContext<TabelSettingModalData>} context */ (
					context
				) => {
					const validateForm = () => {
						return new Promise(async (resolve, reject) => {
							try {
								...
								if (!allowedPageSize.includes(pageSize)) {
									setErrorMessage('Jumlah baris tidak diizinkan');
									resolve(null);
								}
								if (!allowedPageControlPosition.includes(controlPosition)) {
									setErrorMessage('Posisi kontrol tidak diizinkan');
									resolve(null);
								}
					...
					const toggleEditTabelSetting = () => {
						if (editTabelSetting) {
							...
							context.data.setPageControlPosition(
								context?.data?.table?.controlPosition || 0
							);
							if (context?.data?.table?.state) {
								setColumnState(context.data.table.state);
								context.data.setColumnOrder(
									context.data.table.state.map((item, index) => item.id)
								);
								context.data.setColumnVisibility(
									context.data.table.state.reduce((acc, item) => {
					...
					return (
						<Backdrop>
							<Layout
								...
								onEsc={true}
								onEnter={(e, isLayout) => {
									if (isLayout && editTabelSetting) handleSubmitSetting(e);
								}}
								className={`${styles.tabel__setting}`}
							>
								<Head
									title={editTabelSetting ? 'Atur Tabel' : 'Pengaturan Tabel'}
								/>
								...
								<Inner>
									...
									<h3 className={styles.tabel__setting_title}>
										Visibilitas dan Urutan Kolom
									</h3>
									<div
										className={`${styles.tabel__setting_ordering} ${activeId ? styles.active : ''}`}
									>
										{columnState.map((item, index) => (
											<div
												...
												onDragEnd={
													editTabelSetting
														? () => {
																context.data.setColumnOrder(handleSort());
															}
														: null
												}
												onDragOver={
													editTabelSetting ? (e) => e.preventDefault() : null
												}
												key={item.id}
											>
												<div
									...
									<div
										className={`${styles.tabel__setting_control} ${editTabelSetting && activeId ? '' : styles.hide}`}
									>
										<div
											className={
												activeId && canShiftPrev(activeId)
													? ''
													: styles.disabled
											}
											onClick={
										...
										<div
											className={
												activeId && canShiftNext(activeId)
													? ''
													: styles.disabled
											}
											onClick={
									...
									<div className={styles.tabel__setting_select}>
										<div>
											<h3
												...
												className={styles.tabel__setting_title}
											>
												Jumlah Baris
											</h3>
											<select
												id='pageSize'
												className={
													disablePageSizeBorder ? styles.border_off : undefined
												}
												value={pageSize}
												onChange={
													editTabelSetting
										...
										<div>
											...
											<select
												id='controlPosition'
												className={
													disableControlBorder ? styles.border_off : undefined
												}
