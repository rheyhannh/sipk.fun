...
const Profil = () => {
	...
	return (
		<ModalContext.Consumer>
			{
				/** @param {import('@/types/context').ModalContext<ProfilModalData>} context */ (
					context
				) => {
					const validateForm = () => {
						// Validating 'Nama'
						if (isEmpty(nama, { ignore_whitespace: true })) {
							setErrorMessage('Nama lengkap dibutuhkan');
							return false;
						}
						if (!isLength(nama, { min: 6, max: 50 })) {
							setErrorMessage(
								'Nama lengkap minimal 6 karakter maksimal 50 karakter'
							);
							return false;
						}
						if (!isAlpha(nama.replace(/\s/g, ''))) {
							setErrorMessage('Nama lengkap hanya dapat menggunakan huruf');
							return false;
						}
						const fullNameRegex = /^[a-zA-Z]+(\s[a-zA-Z]+)*$/;
						if (!fullNameRegex.test(nama)) {
							setErrorMessage(
								'Nama lengkap hanya dapat menggunakan 1 spasi disetiap kata'
							);
							return false;
						}
						...
						if (isEmpty(nickname, { ignore_whitespace: true })) {
							setErrorMessage('Nickname dibutuhkan');
							return false;
						}
						if (!isLength(nickname, { min: 3, max: 20 })) {
							setErrorMessage(
								'Nickname minimal 3 karakter maksimal 20 karakter'
							);
							return false;
						}

						// Validating 'Jurusan'
						if (isEmpty(jurusan, { ignore_whitespace: true })) {
							setErrorMessage('Jurusan dibutuhkan');
							return false;
						}
						if (!isLength(jurusan, { min: 6, max: 30 })) {
							setErrorMessage(
								'Jurusan minimal 6 karakter maksimal 30 karakter'
							);
							return false;
						}

						// Validating 'Sks Target'
						if (isEmpty(sksTarget, { ignore_whitespace: true })) {
							setErrorMessage('Sks target dibutuhkan');
							return false;
						}
						if (!isInt(sksTarget, { min: 5, max: 1000 })) {
							setErrorMessage(
								'Sks target harus angka bulat (min: 5, max: 1000)'
							);
						}

						// Validating 'Matkul Target
						if (isEmpty(matkulTarget, { ignore_whitespace: true })) {
							setErrorMessage('Matakuliah target dibutuhkan');
							return false;
						}
						if (!isInt(matkulTarget, { min: 5, max: 1000 })) {
							setErrorMessage(
								'Matakuliah target harus angka bulat (min: 5, max: 1000)'
							);
						}

						// Validating 'IPK Target'
						if (isEmpty(ipkTarget, { ignore_whitespace: true })) {
							setErrorMessage('Ipk target dibutuhkan');
							return false;
						}
						if (!isFloat(ipkTarget, { min: 1.0, max: 4.0 })) {
							setErrorMessage('Ipk target harus angka (min: 1.00, max: 4.00)');
						}
						...
						const editProfil = () => {
							return new Promise(async (resolve, reject) => {
								try {
									...
									if (!response.ok) {
										const { toastMessage, refresh, navigate, parsed } =
										...
										if (navigate && navigate?.type === 'push' && navigate?.to) {
											router.push(navigate.to, {
												scroll: navigate?.scrollOptions ?? true
											});
										}
										if (
											navigate &&
					...
					return (
						<Backdrop>
							<Layout
								...
								onEsc={true}
								onEnter={(e, isLayout) => {
									if (isLayout && editProfil) handleEditProfil(e);
								}}
							...
							>
								...
								<Inner>
									...
									<div className={styles.form__input_field}>
										...
										>
											<small>
												{editProfil
													? nama.length
													: context?.data[0]?.fullname?.length}
												/50
											</small>
										</div>
									</div>
										...
										<div className={styles.form__input_field}>
											<div>
												<input
													...
													className={`${styles.form__input} ${styles.max_length}`}
													value={
														editProfil ? nickname : context?.data[0]?.nickname
													}
											...
											<div
												className={`${styles.form__input_length} ${nickname.length >= 20 ? styles.max : ''}`}
											>
												<small>
													{editProfil
														? nickname.length
														: context?.data[0]?.nickname?.length}
													/20
												</small>
											</div>
										</div>
										...
										<div className={styles.form__input_field}>
											<div>
												<input
													...
													className={`${styles.form__input} ${styles.max_length}`}
													value={
														editProfil ? jurusan : context?.data[0]?.jurusan
													}
													...
													required
												/>
												<label htmlFor='jurusan' className={styles.form__label}>
													Jurusan
												</label>
											</div>

											<div
												className={`${styles.form__input_length} ${jurusan.length >= 30 ? styles.max : ''}`}
											>
												<small>
													{editProfil
														? jurusan.length
														: context?.data[0]?.jurusan?.length}
													/30
												</small>
											</div>
										</div>
									...
									<div className={styles.form__input_field}>
										<div>
											<input
												...
												style={{
													cursor: editProfil ? 'not-allowed' : 'default',
													color: editProfil
														? 'var(--infoDark-color)'
														: 'inherit'
												}}
									...
									<h3
										style={{
											marginTop: 'var(--m-0-75)',
											marginBottom: 'var(--m-0-5)',
											textAlign: 'center',
