...
export const Backdrop = ({ children, ...props }) => {
    ...
    return (
        <div className={`${styles.backdrop} ${active ? styles.active : ''}`} {...props}>
            {children}
        </div>
    )
}
 ...
 */
export const Layout = ({ as: Layout = 'div', id = 'modal', onEnter, onEsc = true, children, ...props }) => {
            ...
            onKeyDown={(event) => {
                if (event.key === 'Enter') {
                    if (onEnter && typeof onEnter === 'function') {
                        onEnter(event, event.target === modalRef?.current, modalRef?.current);
                    }
...
export const Head = ({ title, ...props }) => {
    const { handleModalClose, prevModal, handleModalPrev } = React.useContext(ModalContext);
        ...
        <div className={styles.top} {...props}>
            {prevModal &&
                <div
                    ...
                    onClick={(event) => { event.target.blur(); handleModalPrev(); }}
                    onKeyDown={(event) => { if (event.key === 'Enter') handleModalPrev(); }}
                    tabIndex={0}
                >
                    <IoArrowBack />
                </div>
            ...
            <div
                ...
                onClick={(event) => { event.target.blur(); handleModalClose(); }}
                onKeyDown={(event) => { if (event.key === 'Enter') handleModalClose() }}
                tabIndex={0}
            >
                <FaTimes />
            </div>
 ...
 */
export const Button = ({ as: Button = 'div', titleAs: Title = 'h3', title, action, usePreventDefault, ...props }) => {
    const { handleModalClose } = React.useContext(ModalContext);
    ...
    return (
        <Button
            onKeyDown={(event) => {
                if (event.key === 'Enter') {
                    if (usePreventDefault && Array.isArray(usePreventDefault) && usePreventDefault.includes('onKeyDown') && event?.preventDefault) event.preventDefault();
                    if (action) handleAction();
                }
            }}
            onClick={(event) => {
                if (usePreventDefault && Array.isArray(usePreventDefault) && usePreventDefault.includes('onClick') && event?.preventDefault) event.preventDefault();
                if (action) { event.currentTarget.blur(); handleAction(); }
            }}
            tabIndex={0}
            {...props}
        >
