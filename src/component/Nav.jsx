...
export default function Nav({ children }) {
	...
	const { isNavbarActive, setNavbarActive, isRichContent, activeLink } =
				...
				<>
					<div className={styles.content}>
						<aside className='dashboard'>
							<div
								className={`${styles.aside} ${isNavbarActive ? styles.active : ''}`}
								id='aside'
							>
								<div className={styles.sidebar}>
									<div>
										...
										<span className={styles.link} onClick={handleAkunModal}>
											<span className={styles.link__icon}>
												<RiUserSettingsLine size={'24px'} />
											</span>

											<h4 className={styles.link__text}>{'Akun'}</h4>
										</span>
										...
										{dashboardNavItem.map((item, index) => (
											...
											>
												...
												<h4 className={styles.link__text}>{item.text}</h4>
											</Link>
										))}

										<span className={styles.link} onClick={handleRatingModal}>
											<span className={styles.link__icon}>
												<AiOutlineStar size={'24px'} />
											</span>

											<h4 className={styles.link__text}>{'Rating'}</h4>
										</span>

										<span className={styles.link} onClick={handleTentangModal}>
											<span className={styles.link__icon}>
												<GoInfo size={'24px'} />
											</span>

											<h4 className={styles.link__text}>{'Tentang'}</h4>
										</span>
										...
										>
											...
											<h4 className={styles.link__text}>{'Keluar'}</h4>
										</span>
									</div>
								</div>
							</div>
				...
				<>
					...
					>
						<div className={styles.sidebar}>
							<div>
								{data && data.length !== 0 && !error && (
									<div onClick={handleProfilModal} className={styles.profile}>
										<span className={styles.profile__icon}>
											<PiUserCircleLight size={'24px'} />
										</span>
										<h4 className={styles.profile__text}>{data[0].nickname}</h4>
									</div>
								)}
