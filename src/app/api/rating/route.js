...
/**
 * Array of string berisikan method yang tersedia pada route `'/api/rating'`
 */
const routeMethods = ['GET', 'POST', 'PATCH'];

/**
 * Opsi `CORS` yang digunakan pada route `'/api/rating'`
 *
...
/**
 * Route Handler untuk `OPTIONS` `'/api/rating'`
...
export async function POST(request) {
	...
	try {
        ...
        */
		var userUniversitas =
			decryptedSession?.user?.user_metadata?.university ??
			decodedAccessToken?.user_metadata?.university;
		if (!userUniversitas) {
			...
			if (error) {
				throw serverError.interval_server_error(
					...
					undefined,
					{
						severity: 'error',
						reason:
							'Failed to add user rating, cant resolve user universitas, supabase error occurred',
						stack: null,
			...
			if (!data.length || !data[0]?.university) {
				throw notFoundError.resource_not_found(
					...
					undefined,
					{
						severity: 'error',
						reason:
							'Failed to add user rating, cant resolve user universitas after querying',
						stack: null,
						functionDetails: 'supabase.from at POST /api/rating line 186',
						functionArgs: { from: 'user', select: '*' },
						functionResolvedVariable: { data, error },
			...
			if (error) {
				throw serverError.interval_server_error(
					...
					undefined,
					{
						severity: 'error',
						reason:
							'Failed to add user rating, error when trying to resolve user nickname',
						stack: null,
		...
		var { data, error } = await supabase
			.from('rating')
			.insert({ ...formData, owned_by: userId, unix_created_at: unixNow })
			.select();
		if (error) {
			throw serverError.interval_server_error(
				defaultUserErrorMessage,
			...
			if (error) {
				throw serverError.interval_server_error(
					...
					undefined,
					{
						severity: 'error',
						reason:
							'Failed to edit user rating, cant resolve user universitas, supabase error occurred',
						stack: null,
			...
			if (!data.length || !data[0]?.university) {
				throw notFoundError.resource_not_found(
					...
					undefined,
					{
						severity: 'error',
						reason:
							'Failed to edit user rating, cant resolve user universitas after querying',
						stack: null,
						functionDetails: 'supabase.from at PATCH /api/rating line 186',
						functionArgs: { from: 'user', select: '*' },
						functionResolvedVariable: { data, error },
			...
			if (error) {
				throw serverError.interval_server_error(
					...
					undefined,
					{
						severity: 'error',
						reason:
							'Failed to edit user rating, error when trying to resolve user fullname',
						stack: null,
			...
			if (error) {
				throw serverError.interval_server_error(
					...
					undefined,
					{
						severity: 'error',
						reason:
							'Failed to edit user rating, error when trying to resolve user nickname',
						stack: null,
