...
/**
 * Array of string berisikan method yang tersedia pada route `'/api/matkul-history'`
 */
const routeMethods = ['GET', 'DELETE'];

/**
 * Opsi `CORS` yang digunakan pada route `'/api/matkul-history'`
 *
...
const limiter = await rateLimit({
	interval:
		parseInt(process.env.API_MATKULHISTORY_TOKEN_INTERVAL_SECONDS) * 1000,
	uniqueTokenPerInterval: parseInt(
		process.env.API_MATKULHISTORY_MAX_TOKEN_PERINTERVAL
	)
});

/**
 * Route Handler untuk `OPTIONS` `'/api/matkul-history'`
...
export async function GET(request) {
	...
	try {
		...
		if (isService) {
			...
			if (error) {
				const {
					...
					functionDetails:
						'supabaseService.from at GET /api/matkul-history line 55',
		...
		const { data, error } = await supabase
			.from('matkul_history')
			.select('*')
			.order('last_change_at', { ascending: true });
		if (error) {
		...
		if (
			...
			throw badRequestError.malformed_request_params(
				...
				undefined,
				{
					severity: 'error',
					reason:
						"Request param 'id' and 'matkulId' expected exist and typed as string and UUID format",
					stack: null,
		...
		if (error) {
			throw serverError.interval_server_error(
				...
				undefined,
				{
					severity: 'error',
					reason:
						'Failed to delete matakuliah when deleting matakuliah history',
					stack: null,
					functionDetails:
						'supabase.from at DELETE /api/matkul-history line 157',
		...
		if (error) {
			throw serverError.interval_server_error(
				...
				undefined,
				{
					...
					stack: null,
					functionDetails:
						'supabase.from at DELETE /api/matkul-history line 174',
					functionArgs: {
						from: 'matkul_history',
						delete: true,
						eq: { id: id }
