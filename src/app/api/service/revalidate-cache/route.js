...
const limiter = await rateLimit({
    ...
    uniqueTokenPerInterval: parseInt(process.env.API_SERVICE_MAX_TOKEN_PERINTERVAL),
})
    ...
    try {
        ...
        if (isService) {
            if (tagParam || pathParam) {
                if (tagParam) { revalidateTag(tagParam); }
                else { revalidatePath(pathParam); }
