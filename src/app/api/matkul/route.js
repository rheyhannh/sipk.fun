...
export async function PATCH(request) {
	...
	try {
		...
		if (!matkulId || typeof matkulId !== 'string' || !isUUID(matkulId)) {
			throw badRequestError.malformed_request_params(
				...
				undefined,
				{
					severity: 'error',
					reason:
						"Request param 'id' expected exist and typed as string and UUID format",
					stack: null,
        ...
        */
		var userUniversitasId =
			decryptedSession?.user?.user_metadata?.university_id ??
			decodedAccessToken?.user_metadata?.university_id;
			...
			if (error) {
				throw serverError.interval_server_error(
					...
					undefined,
					{
						severity: 'error',
						reason:
							'Failed to edit matakuliah, cant resolve user universitas id, supabase error occurred',
						stack: null,
			...
			if (!data.length || !data[0]?.university_id) {
				throw notFoundError.resource_not_found(
					...
					undefined,
					{
						severity: 'error',
						reason:
							'Failed to edit matakuliah, cant resolve user universitas id after querying',
						stack: null,
						functionDetails: 'supabase.from at PATCH /api/matkul line 186',
						functionArgs: { from: 'user', select: '*' },
		...
		var { data: universitas, error } = await supabase
			.from('universitas')
			.select('*')
			.eq('id', userUniversitasId);
		...
		var { data: matkulUpdated, error } = await supabase
			.from('matkul')
			.update({ ...formData, updated_at: unixNow })
			.eq('id', matkulId)
			.select();
		if (error) {
			const updateError = error;
			var { error: rollbackError } = await supabase
				.from('matkul_history')
				.update(originalMatkulHistory)
				.eq('matkul_id', matkulId);
			throw serverError.interval_server_error(
				defaultUserErrorMessage,
				undefined,
				{
					...
					reason:
						"Failed to edit matakuliah when trying to update matakuliah. Will attempting to rollback matkul history to previous state. If rollback failed, error showed in 'more'",
					stack: null,
		...
		return cors(
			request,
			NextResponse.json(
				{ matkul: matkulUpdated[0], ref: matkulHistory[0] },
...
export async function DELETE(request) {
	...
	try {
		...
		if (!data.length) {
			throw notFoundError.resource_not_found(
				...
				undefined,
				{
					severity: 'error',
					reason:
						'Failed to delete matakuliah, cant resolve matakuliah history',
					stack: null,
		...
		if (error) {
			throw serverError.interval_server_error(
				...
				undefined,
				{
					severity: 'error',
					reason:
						'Failed to delete matkauliah, cant resolve matakuliah history',
					stack: null,
					functionDetails: 'supabase.from at DELETE /api/matkul line 319',
					functionArgs: {
						from: 'matkul_history',
		...
		var { data: matkulHistory, error } = await supabase
			.from('matkul_history')
			.update({ current: updatedCurrent, last_change_at: unixNow })
			.eq('matkul_id', matkulId)
			.select();
		...
		if (error) {
			...
			throw serverError.interval_server_error(
				...
				undefined,
				{
					severity: rollbackError ? 'critical' : 'error',
					reason:
						"Failed to delete matakuliah when trying to delete matakuliah. Will attempting to rollback matkul history to previous state. If rollback failed, error showed in 'more'",
					stack: null,
...
export async function POST(request) {
	...
	try {
		...
		if (ref && (typeof ref !== 'string' || !isUUID(ref))) {
			throw badRequestError.malformed_request_params(
				...
				undefined,
				{
					severity: 'error',
					reason:
						"Request param 'ref' expected typed as string and UUID format",
					stack: null,
        ...
        */
			...
			if (error) {
				throw serverError.interval_server_error(
					...
					undefined,
					{
						severity: 'error',
						reason:
							'Failed to add matakuliah, cant resolve user universitas id, supabase error occurred',
						stack: null,
			...
			if (!data.length || !data[0]?.university_id) {
				throw notFoundError.resource_not_found(
					...
					undefined,
					{
						severity: 'error',
						reason:
							'Failed to add matakuliah, cant resolve user universitas id after querying',
						stack: null,
						functionDetails: 'supabase.from at POST /api/matkul line 186',
						functionArgs: { from: 'user', select: '*' },
						functionResolvedVariable: { data, error },
		...
		/** @type {SupabaseTypes._from<SupabaseTypes.MatkulData>} */
		var { data: matkulBaru, error } = await supabase
			.from('matkul')
			.insert({ ...formData, owned_by: userId, created_at: unixNow })
			.select();
		if (error) {
			throw serverError.interval_server_error(
				defaultUserErrorMessage,
		...
		var { data: matkulBaruHistory, error } = ref
			? await supabase
					.from('matkul_history')
					.update({
						matkul_id: matkulBaru[0].id,
						current: { ...formData, type: 'tambah', stamp: unixNow },
						prev: null,
						owned_by: userId,
						last_change_at: unixNow
					})
		...
		if (!matkulBaruHistory.length || error) {
			...
			throw notFoundError.resource_not_found(
				...
				undefined,
				{
					severity: rollbackError ? 'critical' : 'error',
					reason:
						"Failed to add matakuliah, ref not exist. Will attempting to remove added matakuliah, if error showed in 'more'",
					stack: null,
		...
		return cors(
			request,
			NextResponse.json(
				{ matkul: matkulBaru[0], ref: matkulBaruHistory[0] },
				{ status: 200, headers: responseHeaders }
			),
			routeCorsOptions
		);
