...
const limiter = await rateLimit({
	...
	uniqueTokenPerInterval: parseInt(
		process.env.API_REGISTER_MAX_TOKEN_PERINTERVAL
	)
});
...
export async function POST(request) {
	...
	try {
		...
		if (users.length >= maximumRegisteredUser) {
			throw serverError.user_registration_closed(undefined, undefined, {
				severity: 'warning',
		...
		var { data: universitas, error } = await supabaseService
			.from('universitas')
			.select('*')
		...
		const userUniversitasName = universitas.find(
			(item) => item.id === formData.university_id
		)?.nama;
		...
		if (signUpError) {
			if (
				...
				throw serverError.service_unavailable(undefined, undefined, {
					severity: 'error',
					reason:
						"Failed to register user, supabase error occurred, see details in 'more'",
					stack: null,
					functionDetails:
						'supabase.auth.signUp at POST /api/register line 200',
			...
			throw serverError.interval_server_error(
				...
				undefined,
				{
					...
					stack: null,
					functionDetails:
						'supabase.auth.signUp at POST /api/register line 214',
					functionArgs: { formData },
					functionResolvedVariable: null,
					request: await getRequestDetails(),
					more: signUpError
