...
const limiter = await rateLimit({
	interval:
		parseInt(process.env.API_AUTH_CONFIRM_TOKEN_INTERVAL_SECONDS) * 1000,
	uniqueTokenPerInterval: parseInt(
		process.env.API_AUTH_CONFIRM_MAX_TOKEN_PERINTERVAL
	)
});
...
export async function GET(request) {
	...
	try {
		...
		if (error) {
			throw authError.invalid_hash_token(undefined, undefined, {
				severity: 'error',
				reason: "Supabase error occurred, see details in 'more'",
				stack: null,
				functionDetails:
					'supabase.auth.verifyOtp at GET /api/auth/confirm/signup line 103',
		...
		if (!data?.session?.user?.id) {
			throw notFoundError.resource_not_found(undefined, undefined, {
				severity: 'error',
				reason:
					'Failed to update user data when confirming signup, user id not exist',
				stack: null,
		...
		var { error } = await supabase
			.from('user')
			.update({ is_email_confirmed: true, email_confirmed_at: dateNow })
		...
		if (error) {
			throw serverError.interval_server_error(undefined, undefined, {
				...
				stack: null,
				functionDetails:
					'supabase.from at GET /api/auth/confirm/signup line 143',
				functionArgs: {
					from: 'user',
					update: { is_email_confirmed: true, email_confirmed_at: dateNow },
					eq: { id: data.session.user.id }
