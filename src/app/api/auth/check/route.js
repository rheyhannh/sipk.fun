...
const limiter = await rateLimit({
	...
	uniqueTokenPerInterval: parseInt(
		process.env.API_AUTH_CHECK_MAX_TOKEN_PERINTERVAL
	)
});
	...
	try {
		...
		if (error) {
			throw authError.invalid_session(undefined, undefined, {
				...
				stack: null,
				functionDetails:
					'supabase.auth.getSession at GET /api/auth/check line 83',
				functionArgs: null,
				functionResolvedVariable: null,
				request: await getRequestDetails(),
				more: error
		...
		if (data?.session) {
			if (
				!serviceUserIdCookie ||
				serviceUserIdCookie !== data.session.user.id
			) {
				cookieStore.set({
					name: 's_user_id',
			...
			if (
				!serviceAccessTokenCookie ||
				serviceAccessTokenCookie !== data.session.access_token
			) {
				cookieStore.set({
					name: 's_access_token',
